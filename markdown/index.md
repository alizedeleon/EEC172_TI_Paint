...
abstract: 'TI Paint is designed as a drawing tool to allow its user to paint
a picture on an OLED screen by physically moving the accelerometer on a
CC3200 LaunchPad. This tool allows options to customize the paintbrush as the
user draws as well as to save the current painting to retrieve the last save
work. The tool's main features include movement-controlled drawing, where
